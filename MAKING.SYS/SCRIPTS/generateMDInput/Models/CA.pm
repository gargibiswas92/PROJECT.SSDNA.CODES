...
#####################
#this file uses the following data structures
#
#$mdDataRef
# {ANGLES}
# {BEADS} ->array[BEAD]
...
#  ==========================================================
      my $rescBin;
      my $runningDir;
      system(`gcc -c $scriptDirectory/resc/resc.c`);
      system(`gfortran -std=legacy resc.o $scriptDirectory/resc/prep.f $scriptDirectory/resc/rescprep.f $scriptDirectory/resc/resccal.f -o resc`);
      $runningDir = cwd();
      $rescBin = $runningDir.'/resc';
      print $rescBin;
# ===========================================================
    #   if (-e '/usr/lib64/libg2c.so.0') {
    #  # file exists, run the original script
    #      $rescBin = $scriptDirectory.'/resc';
    #      }
    #   else {
    #          # file does not exist, run recompiled for rocky
    #       $rescBin = $scriptDirectory.'/rockyResc/resc';
    #       }
# ===========================================================
...
sub determineDNABeads
{
   ...
   # my $atomType;
    ...
    foreach my $atom (@pdbfile)  {
        ...
        # while at the same residue $lastRes=$curentRes, when we get to the next residue
        # $lastRes ne $curentRes 
...
sub determineBonds
{
  ...
  $DynamicRangeLong = @DynamicRange;
  my $bondIter = -1;
  while ($i < $DynamicRangeLong)
  {
    #for (my $residueIter = 0; $residueIter < scalar(@$mdResiduesRef)-1; $residueIter++)
    for (my $residueIter = $DynamicRange[$i]-1; $residueIter < $DynamicRange[$i+1]-1; $residueIter++)
    {
        $bondIter++;
        my $CACAbondRef = {};      
...
sub determineAngles
{
  ...
  if ($executionPreferencesRef->{"HAS_STATIC_ATOMS"} =~ /YES/)
  {
    ...
    {
        if ($DynamicRange[$counter] != 1) {
                $DynamicRange[$counter] = $DynamicRange[$counter] -2;
        }
        if ($DynamicRange[$counter+1] != scalar(@$mdResiduesRef)) {
                $DynamicRange[$counter+1] = $DynamicRange[$counter+1] + 2;
        }
        $counter = $counter +2;
    }
  } else {
  ...
  while ($i < $DynamicRangeLong)
  {
    for (my $residueIter = $DynamicRange[$i]-1; $residueIter < $DynamicRange[$i+1]-2; $residueIter++)
    {
        $angleIter++;
        my $CACACAangleRef = {};
...
sub determineDihedrals
{
  ...
  if ($executionPreferencesRef->{"HAS_STATIC_ATOMS"} =~ /YES/)
  {
    ...
    {
        if ($DynamicRange[$counter] != 1) {
                $DynamicRange[$counter] = $DynamicRange[$counter] -3;
        }
        if ($DynamicRange[$counter+1] != scalar(@$mdResiduesRef)) {
                $DynamicRange[$counter+1] = $DynamicRange[$counter+1] + 3;
        }
        $counter = $counter +2;
    }
  } else {
  ...
  while ($i < $DynamicRangeLong)
  {
    for (my $residueIter = $DynamicRange[$i]-1; $residueIter < $DynamicRange[$i+1]-3; $residueIter++)
    {
        $dihedralIter++;
...
sub determineContacts
{
  ...
  if ($executionPreferencesRef->{"INTER_CHAIN_CONTACTS"} =~ m/NO/)
  {
    foreach my $pdbChainRef (@{$pdbDataRef->{"CHAINS"}})
    {
      ...
      if ($executionPreferencesRef->{"HAS_STATIC_ATOMS"} =~ /YES/)
      {
        ...
        @DynamicBeadRange = (1,scalar(@{$pdbDataRef->{"RESIDUES"}}));
      }
      my $i = 0;
      my $csu='NO';
      ...
      {
        if (($pdbChainRef->{"RESIDUES"}->[0]->{"RESIDUE_ID"} <= $DynamicBeadRange[$i+1]) && ($pdbChainRef->{"RESIDUES"}->[0]->{"RESIDUE_ID"} >= $DynamicBeadRange[$i]))
        {
         $csu='YES';
         for (my $residueIter = $DynamicBeadRange[$i]-1; $residueIter < $DynamicBeadRange[$i+1]; $residueIter++)
         {
            ...
            # `$scriptDirectory/resc csu.input.$currentChainID $currentResidueID >> csu.output.$currentChainID.$currentResidueID`;
             `$rescBin csu.input.$currentChainID $currentResidueID >> csu.output.$currentChainID.$currentResidueID`;
  ...
  else
  {
    ...
    # system(`sed "s/ P /   /g" < NK2_prot.pdb > csu.input.A`);
    if ($executionPreferencesRef->{"HAS_STATIC_ATOMS"} =~ /YES/)
    {
        my $DynamicRangeList = $executionPreferencesRef->{"DYNAMIC_ATOM_RANGE"};
        @DynamicRange = split /,/, $DynamicRangeList;
      ...
      @DynamicRange = [1,scalar(@{$pdbDataRef->{"RESIDUES"}})];
    }
    my $i = 0;
    $DynamicRangeLong = @DynamicRange;
    my $bondIter = -1;
    while ($i < $DynamicRangeLong)
    {
            for (my $residueIter = $DynamicRange[$i]; $residueIter <= $DynamicRange[$i+1]; $residueIter++)
            {
                # `$scriptDirectory/resc csu.input.A $residueIter >> csu.output.A.$residueIter`;
                `$rescBin csu.input.A $residueIter >> csu.output.A.$residueIter`;
...
#  if ($executionPreferencesRef->{"HAS_STATIC_ATOMS"} =~ /YES/)
#  {
#      my $DynamicList = $executionPreferencesRef->{"DYNAMIC_ATOM_RANGE"};
#      @DynamicRange = split /,/, $DynamicList;
#  } else {
#    my @DynamicRange = (1,scalar(@allBeads));
#  }
  #my $i = 0;
  #my $DynamicLong = @DynamicRange;
  #while ($i < $DynamicLong)
  #{
  
    my $lastDynamicBead = scalar(@allBeads);
    for (my $firstBeadIter = 0; $firstBeadIter < $lastDynamicBead; $firstBeadIter++)
      {
        ...
        for (my $secondBeadIter = $firstBeadIter+1; $secondBeadIter < scalar(@allBeads); $secondBeadIter++)
        {
          ...
          #my $IsDynamicFirst = isDynamic($firstResidueID,$executionPreferencesRef);
          #my $IsDynamicSec = isDynamic($secondResidueID,$executionPreferencesRef);
          
#         if($firstBeadID <= 448 and $secondBeadID <=448)
#          {next;
#          }
#	  print "$firstBeadID,$secondBeadID \n";
	  ...
	  and (!defined($mdDataRef->{"CONTACTS"}->{$firstBeadID.".".$secondBeadID}))
          and ($firstBeadID <=$lastDynamicBead or  $secondBeadID <=$lastDynamicBead )
          and (isDynamic($firstResidueID,$executionPreferencesRef) or isDynamic($secondResidueID,$executionPreferencesRef))
          and (!defined($mdDataRef->{"DNA.CONTACTS"}->{$firstBeadID.".".$secondBeadID})))
          #and ($IsDynamicFirst or $IsDynamicSec))
          {
...
sub isDynamic
{
    my ($ResidueID,$executionPreferencesRef) =@_;
    my $IsDynamic = 0;
    my $DynamicRangeList = $executionPreferencesRef->{"DYNAMIC_ATOM_RANGE"};
    my @DynamicRange = split /,/, $DynamicRangeList;
    my $DynamicRangeLong = @DynamicRange;
    my $i = 0;
    while ($i < $DynamicRangeLong)
    {
        if ($ResidueID >= $DynamicRange[$i] and $ResidueID <= $DynamicRange[$i+1])
        {
            $IsDynamic = 1;
        }
        $i = $i + 2;
